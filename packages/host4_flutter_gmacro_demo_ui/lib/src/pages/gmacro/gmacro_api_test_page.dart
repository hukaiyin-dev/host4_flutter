  ...
  List<_Section> _buildSections() {
    ...
    return [
      _Section(
        ...
        items: [
          ...
          _Item(
            'switchToSettingMode',
            '切换配置触点映射',
            () => s!.switchToSettingMode(),
          ),
          _Item(
            'switchToTouchMappingMode',
            '切换触点映射模式',
            () => s!.switchToTouchMappingMode(),
          ),
        ],
      ),
      _Section(
      ...
      _Section(
        title: 'Other / 新增接口',
        items: [
          _Item('queryVibrateOpen', '查询马达开关状态', () => s!.queryVibrateOpen()),
          _Item(
          ...
          _Item('queryWorkStyle', '查询手柄工作模式', () => s!.queryWorkStyle()),
          _Item(
            'switchWorkStyle',
            '设置手柄工作模式(mode=1)',
            () => s!.switchWorkStyle(mode: 1),
          ),
          _Item('queryOutputMode', '查询当前手柄模式', () => s!.queryOutputMode()),
          _Item(
            'switchOutputMode',
            '设置当前手柄模式(mode=1)',
          ...
          _Item(
            ...
            '查询灯效（Pantas 原生直通）',
            () => _rawInvoke('queryLightingEffectPantas'),
          ),
          _Item(
      ...
      _Section(
        ...
        items: [
          ...
          _Item('endMacroConfig', '结束配置', () => s!.endMacroConfig()),
          _Item(
            'queryCurrentMacro',
            '查询当前宏 (profile=0)',
      ...
      _Section(
        ...
        items: [
          _Item(
            'setScreenSize',
            '设置触屏映射屏幕尺寸',
            () => s!.setScreenSize(orientation: 0, width: 1920, height: 1080),
          ),
          _Item(
            'setKeyMapping',
            '设置触屏单键映射',
          ...
          _Item(
            'setMacroKeyTrigger',
            '设置触屏宏触发方式',
            () => s!.setMacroKeyTrigger(
              keyCode: GamepadKey.m1.value,
              touchType: 0,
            ),
          ),
          _Item(
            'setMacroTerminationKey',
            '设置触屏宏终止按键',
            () => s!.setMacroTerminationKey(
              keyCode: GamepadKey.m1.value,
              terminateKey: GamepadKey.b.value,
            ),
          ),
          _Item(
            'setKeyTurboSpeed',
            '设置触屏映射连发速率',
            () => s!.setKeyTurboSpeed(keyCode: GamepadKey.a.value, turbo: 10),
          ),
          _Item(
            'keyMappingEnd',
            '结束触屏映射配置',
            () => s!.keyMappingEnd(page: 0, packet: 0),
          ),
        ],
      ),
      _Section(
...
class _ApiTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (r == null) {
      ...
      if (!enabled) {
        tileColor = colors.surfaceContainerHighest.withValues(alpha: 0.35);
      }
      ...
      tileColor = colors.errorContainer.withValues(alpha: 0.4);
