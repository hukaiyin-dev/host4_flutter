    ...
    fun toMap(rsp: MacroHandleConfigRsp): Map<String, Any?> {
        ...
        return mapOf(
            ...
            "leftStickX" to mapOf(
                ...
                "sensitivity" to (rocker?.x_sensitivityLeft ?: 0),
                "reverse" to if (rocker?.isX_isExchangeLeft == true) 1 else 0,
            ),
            "leftStickY" to mapOf(
                "start" to (rocker?.y_startLeft ?: 0),
                "end" to (rocker?.y_terminationLeft ?: 0),
                "sensitivity" to (rocker?.y_sensitivityLeft ?: 0),
                "reverse" to if (rocker?.isY_isExchangeLeft == true) 1 else 0,
            ),
            "rightStickX" to mapOf(
                "start" to (rocker?.x_startRight ?: 0),
                "end" to (rocker?.x_terminationRight ?: 0),
                "sensitivity" to (rocker?.x_sensitivityRight ?: 0),
                "reverse" to if (rocker?.isX_isExchangeRight == true) 1 else 0,
            ),
            "rightStickY" to mapOf(
                "start" to (rocker?.y_startRight ?: 0),
                "end" to (rocker?.y_terminationRight ?: 0),
                "sensitivity" to (rocker?.y_sensitivityRight ?: 0),
                "reverse" to if (rocker?.isY_isExchangeRight == true) 1 else 0,
            ),
            "leftStickKey" to mapOf(
                "deadZoneShape" to (rocker?.deathZoneLeft ?: 0),
                "maxOutput" to (rocker?.maxOutLeft ?: 0),
                "curveApply" to (rocker?.curveApplyLeft ?: 0),
                "curveApplyKey" to (rocker?.curveKeyLeft ?: 0),
                "lineCorrection" to  if (rocker?.isLineCorrectionLeft == true) 1 else 0,
                "pointCount" to (rocker?.pointCountLeft ?: 0),
                "points" to points(rocker?.leftPointList),
            ),
            "rightStickKey" to mapOf(
                "deadZoneShape" to (rocker?.deathZoneRight ?: 0),
                "maxOutput" to (rocker?.maxOutRight ?: 0),
                "curveApply" to (rocker?.curveApplyRight ?: 0),
                "curveApplyKey" to (rocker?.curveKeyRight ?: 0),
                "lineCorrection" to  if (rocker?.isLineCorrectionRight == true) 1 else 0,
            ...
            "motion" to mapOf(
                ...
                "yReverse" to if (motion?.isMotionYaxisSwitch == true) 1 else 0,
                "motionSwitch" to if (motion?.isMotionSwitch == true) 1 else 0,
                "mappingSwitch" to if (motion?.isMotionMapperSwitch == true) 1 else 0,
                "triggerMode" to (motion?.motionMethod ?: 0),
                "triggerKey" to (motion?.motionKey ?: 0),
                "deadZone" to (motion?.motionDeathZone ?: 0),
                "mapping" to  (motion?.motionMapperModel ?: 0),
            ),
        )
    }
