...
class _EventLogState extends State<_EventLog> {
  ...
  void initState() {
    ...
    debugPrint(
      '[GmacroInputDebugPage._EventLog] initState — subscribing to states & buttonEvents',
    );

    // 订阅完整状态帧：按键、摇杆、扳机变化时记录
    _stateSub = widget.service.states.listen((state) {
      debugPrint(
        '[GmacroInputDebugPage._EventLog] state emitted: source=${state.source.name} keys=${state.pressedKeys.map((e) => e.name)}',
      );
    ...
    _buttonSub = widget.service.buttonEvents.listen((event) {
      debugPrint(
        '[GmacroInputDebugPage._EventLog] buttonEvent: ${event.key.name} ${event.phase.name}',
      );
